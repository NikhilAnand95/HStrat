...
public class Simulation_ESP 
{
	...
	public int infected_based_HCU_selection(int time, int hcu_id, int pos, double[][] trans_HCUs_dist, int[] num_infect_sample) throws IOException 
	{
		...
		for(int i=0; i<nodes; i++)
		{
			fw.write(num_infect_sample[i] + "  ");
			total_infected = total_infected +num_infect_sample[i];			
		}
		...
		{			
				...
				for(int j=0; j<nodes; j++)
				{
					trans_HCUs_dist[i][j] = (double) (num_infect_sample[j])/total_infected;
	...
	/* selecting position of HCU for next time step based on population of it's neighbor */
	public int infected_based_HCU_selection(int time, int hcu_id, int pos, int[][] adj_matrix, int [] num_infect_node) 
	{
		int max_pop = 0;
		int max_nbr = pos;			
	...
	}/*  End of infected_based_HCU_selection() */
	...
	public void next_positions_of_HCUs(int time, int [] curr_pos_HCUs, double[][] init_HCUs_dist, int[][] adj_matrix, double[][] trans_HCUs_dist, int [] degree_node) throws IOException 
	{
			...
			{	
				...
				{	
					...
					curr_pos_HCUs[i] = infected_based_HCU_selection(time, i, pos, trans_HCUs_dist, num_infect_sample);
				}
