...
public class Simulation_ESP 
{
    ...
    private int [] num_suscepted_city;
    private int [] num_infected_city;
    private int [] num_recovered_city;
    
    //private String query="isInfected(100)";
	...
	Simulation_ESP(int _nodes, int _simTime, int _people, int _hcus, int _infected, int _sampleSize, double _localProb, double _infThreshold, double _beta, double _delta, String _positionFolder, String _stateFolder, String _mappingFolder, String _outputFolder)
	{
		...
		outputFile     = _outputFolder +"SMC/" + (hcus)+"_hcus.txt";
		outputFile1    =  _outputFolder + "SMC/SIR_"+(local_prob)+"_"+(inf_threshold)+"_"+(sample_size)+"_"+(delta)+"_smc.txt";
	...
	public void find_infected_at_each_node(String outputFile, int[] num_infect_node, int[] curr_pos_people) throws IOException  
	{
		...
		for(int i=0; i<nodes; i++)
		{
			...
			num_suscepted_city[i] = 0;
			num_infected_city[i] = 0;
			num_recovered_city[i] = 0;
			...
			for(int j=1; j<=num_pop_node[i][0];j++)
			{
				...
				else
				{
					...
					{	
						...
						num_recovered_city[i]++;					
					}
					else
					{
						num_suscepted_city[i]++;
					}
			    }
			}			
			num_infect_node[i] = count;
			num_infected = num_infected + count;			
			fw.write( " -- " + num_infect_node[i] + "\n");
			System.out.println("node id : " + i + " Suscepted : " + num_suscepted_city[i] + " Infected : " +num_infected_city[i] + " Recovered : " + num_recovered_city[i] );
		 }		
		fw.close();
	...
	public void initial_positions_of_people(double [][] init_pop_dist) throws IOException 
	{
		...
		num_suscepted_city= new int[nodes];
		num_infected_city =  new int[nodes];
		num_recovered_city =  new int[nodes];
	...
	public boolean start_simulation(double [][] init_pop_dist, double [][] init_HCUs_dist, int [][] adj_matrix, int [][] init_infected, double [][] trans_pop_dist, double [][] trans_HCUs_dist, int [] degree_node, HashMap<Integer, ArrayList<Integer>> hm) throws IOException 
	{
    	...
    	//int agent = (int) (people * Math.random()) + 1;
    	//int city  = (int) (nodes * Math.random()) + 1;
    	
    	pq = new ProcessQuery();
      	sim_time = pq.read_query();
    	return run_simulation(init_pop_dist, init_HCUs_dist, adj_matrix, trans_pop_dist, trans_HCUs_dist, degree_node);
	...
	/* selects the position of HCU for next time unit/step randomly based on connectivity, Function Parameters : (time, time_stamp_HCUs, curr_pos_HCUs, num_HCUs_node, init_HCUs_dist, adj_matrix, trans_HCUs_dist); */
	public void next_positions_of_HCUs(int time, int [] curr_pos_HCUs, double[][] init_HCUs_dist, int[][] adj_matrix, double[][] trans_HCUs_dist, int [] degree_node) 
	{
		    System.out.println();
			...
			{	
				...
				{	
					curr_pos_HCUs[i] = random_based_HCU_selection(time, i, pos, trans_HCUs_dist);				
				}
	...
	public void store_final_results(int time) throws IOException 
	{
		...
		String fileName = outputFolder + "time-ran.txt";
	...
	public boolean run_simulation(double [][] init_pop_dist, double [][] init_HCUs_dist, int [][] adj_matrix, double [][] trans_pop_dist, double [][] trans_HCUs_dist, int [] degree_node) throws IOException 
	{
        // sim_time = 20;
        ...
        for(int j=1; j <=sim_time; j++)		
	  	{ 
        	...
        	boolean isValid  = pq.check_query_satisfied(curr_state_people, num_suscepted_city, num_infected_city, num_recovered_city);
        	
        	if(isValid)
        	{
        		System.out.println("Yes Query Satisfied");
        		return true;   	
        	}
